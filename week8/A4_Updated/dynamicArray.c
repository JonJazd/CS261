...
void adjustHeap(DynamicArray* heap, int last, int position, compareFunction compare)
{
    ...
    int left = min*2+1;
    int right = min*2+2;
    if (left<last && compare(heap->data[left],heap->data[min])==-1){
        min = left;
    }
    if (right<last && compare(heap->data[right],heap->data[min])==-1 && compare(heap->data[right],heap->data[left])==-1){
        min = right;
    }
    if (min!=position){
    ...
    if (position<last){
        adjustHeap(heap,last,position+1,compare);
    }
}
...
{
    /*int a = 0;
    while (a*2+1<heap->size){
        TYPE * max = heap->data[a];
        if (compare(heap->data[a*2+1],max)==1){
            max = heap->data[a*2+1];
            dySwap(heap,a,a*2+1);
        }
        if (a*2+2<heap->size){
            if (compare(heap->data[a*2+2],max)==1){
                dySwap(heap,a,a*2+2);
            }
        }
        a++;
    ...
    int a = heap->size/2;
    while (a>-1){
        adjustHeap(heap,heap->size,a,compare);
        a -=1;
    }
}
...
void dyHeapSort(DynamicArray* heap, compareFunction compare)
{
    ...
    while (newSize>0){
        dySwap(heap,newSize,0);
        adjustHeap(heap,newSize-1,0,compare);
        newSize -= 1;
    }
}
